	...
	public String Gis(ModelMap modelMap) {
		...
		if (midstreamLanUseSceVos != null) {
			for (int i = 0; i < midstreamLanUseSceVos.size(); i++) {
				...
				midStreamWetLand = midStreamWetLand + Double.parseDouble(midstreamLanUseSceVos.get(i).getWetlandArea());
			}
		}
...
//			List<CountryIdAndFarmAreaVo> CountryIdAndFarmAreaVos2 = new ArrayList<CountryIdAndFarmAreaVo>();
			for (int i = 0; i < midStreamTbSocioEconomyScePara.size(); i++) {
				double percentArea = (Double.parseDouble(midStreamTbSocioEconomyScePara.get(i).getFldSprinkingArea())
						/ midStreamsprink) * Double.parseDouble(midStreamDripIrrigation);
				CountryIdAndFarmAreaVo countryIdAndFarmAreaVo = new CountryIdAndFarmAreaVo();
				countryIdAndFarmAreaVo.setProjectId(projectId);
